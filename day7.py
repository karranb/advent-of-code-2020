...
def parse_line(line):
    container, contains = (
        line.replace("bags", "").replace("bag", "").replace(" .", "").split("contain")
    )
    contained_bags = map(lambda x: x.strip().split(" ", 1), contains.split(","))
    return [container.strip(), contained_bags]


parsed_input = list(map(parse_line, input.split("\n")))
index = {
    line[0]: {
        contained_bags[1]: contained_bags[0]
...
bags = [container for container in index.keys()]
...
def contains_shiny_gold(bag):
    if bag not in index:
        return False
    bag_index = index[bag]
    if "shiny gold" in bag_index:
        return True
    bags = index[bag].keys()
    return any([contains_shiny_gold(other_bag) for other_bag in bags])


print("part 1", len(filter(contains_shiny_gold, bags)))


def contained_bags_count(bag):
    bag_index = index[bag]
    if len(bag_index) == 0:
        return 0
    return sum(
        [
            int(multiplier) + (int(multiplier) * contained_bags_count(other_bag))
            for other_bag, multiplier in bag_index.items()
        ]
    )


print("part 2", contained_bags_count("shiny gold"))
