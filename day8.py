...
def execute_program(given_instructions):
    ...
    acc = 0
    current_index = 0
    while current_index < len(given_instructions):
        if current_index in index.keys():
            return False, acc
        index[current_index] = 1
        cmd, value = given_instructions[current_index]
        parsed_value = int(value)
        if cmd == JUMP:
            current_index = current_index + parsed_value
            continue
        if cmd == ACC:
            acc += parsed_value
        current_index += 1
...
def do_next_change(_changed_instructions, _instructions):
    any_change = False
    new_instructions = []
    changed_instructions = _changed_instructions
    for i, instruction in enumerate(_instructions):
        if instruction[0] == ACC or any_change:
            new_instructions += [instruction]
            continue
        if i in changed_instructions:
            new_instructions += [instruction]
            continue
        any_change = True
        changed_instructions += [i]
        if instruction[0] == JUMP:
            new_instructions += [(NOOP, instruction[1])]
            continue
