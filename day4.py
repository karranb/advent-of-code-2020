batch = """hcl:#6b5442 ecl:brn iyr:2019
...
def document_has_fields(document):
    return all(field in document for field in required_fields)


documents = map(lambda x: x.replace("\n", " "), batch.split("\n\n"))

part1_documents = filter(document_has_fields, documents)
...
def document_has_valid_fields(document):
    fields = map(lambda x: map(lambda y: y.strip(), x.split(":")), document.split(" "))
    for field in fields:
        [id, value] = field
