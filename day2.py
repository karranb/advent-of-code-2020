input = """15-16 m: mhmjmzrmmlmmmmmm
5-6 d: dcdddhzld
3-4 s: vqssdcbl
3-6 b: bbhbbbqbbb
10-11 q: cqqntqhqwwh
...
def parse(item):
    min, rest_of_item = item.split("-")
    max, raw_char, passw = rest_of_item.split(" ")
    char = raw_char[0]
    return int(min), int(max), char, passw


def get_char_occurrences(char, passw):
    return filter(lambda passw_char: passw_char == char, passw)


def is_valid(item):
    min, max, char, passw = parse(item)
    char_occurrences = get_char_occurrences(char, passw)
    return min <= len(char_occurrences) <= max

valid_passwords = filter(is_valid, inputs)

print("part 1", len(valid_passwords))

def is_valid_part_2(item):
    p1, p2, char, passw = parse(item)
    return (passw[p1 - 1] == char and passw[p2 - 1] != char) or (
        passw[p1 - 1] != char and passw[p2 - 1] == char
    )

valid_passwords = filter(is_valid_part_2, inputs)

print("part 2", len(valid_passwords))
